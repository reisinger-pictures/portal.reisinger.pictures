...
export default function GalleryGroupCouponsTab({ groupId }: Props) {
    const { showToast, confirm } = useUI();
    const { user } = useAuth();
    const swrKey = `/api/management/gallery-groups/${groupId}/coupons`;
    ...
    const handleSave = async (payload: Partial<Coupon>) => {
        try {
            await apiMutate(`/api/management/gallery-groups/${groupId}/coupons`, 'POST', {
                ...payload,
        ...
        <div>
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                <div className="text-sm opacity-70">
                    {coupons.length === 0
                        ? <Trans>Keine Coupons für diese Galerie-Gruppe.</Trans>
                        : t`${couponsLength} Coupon${couponsPlural} für diese Galerie-Gruppe`}
                </div>
                        ...
                        {coupons.length === 0 && (
                            <tr>
                                <td colSpan={6} className="text-center py-10 opacity-50">
                                    <Trans>Keine Coupons für diese Galerie-Gruppe vorhanden.</Trans>
                                </td>
                            </tr>
                        )}
                    </tbody>
