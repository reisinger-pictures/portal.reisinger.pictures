...
- [ ] **FINAL-9 — offen, braucht User-Sign-off:** CTR-7-Join-Impersonation. Jeder mit gültigem Join-Link kann als noch nicht registrierter Dritter joinen und erhält dessen `personal_token`; E-Mail-Besitz wird nicht verifiziert. Ein neuer Test pinnt dies als bewusste Produktentscheidung, aber ein Signier-Credential auf den Namen der Gegenpartei zu erhalten ist eine **bewusste Freigabe**, keine stille Nebenwirkung. **Entscheidung einholen oder als akzeptiertes Risiko in §8 eintragen.**

**Dependabot-Stand (2026-09-26):** 7 Alerts (4 high). Die 3 PRs (#13 `@tiptap/core` 3.30.5, #14 `browserslist` 4.29.0, #15 `baseline-browser-mapping` 2.11.25) waren vom 19.09. und zeigten Check-Fehler **aus der Image-Pull-Ära** (Frontend-Check lief 22s) — bedeutungslos. Mit `gh pr update-branch` auf `main` nachgezogen, damit der Bump gegen den echten Stand getestet wird. Erster frischer Befund: **`Frontend (Lint, Build, Vitest)` und `CI security contract` sind grün** — kein Bump bricht den Build. Merges erst nach vollständigem Lauf.
