...
**Aktive Ausführungswellen im Working Tree (noch nicht committed):**
- [ ] **CR-PAY-010:** Invoice/Free/Quote serverseitig idempotent; PHPUnit-Regressionen für Duplicate/Replay/Conflict/Actor-/Brand-Isolation, danach fokussierter Vitest-Nachweis.
- [ ] **CR-FE-030:** Child-/Gruppen-Lizenzierung mit gemischten Descriptoren; Vitest-/Komponenten-Regression und getaggte Playwright-Abdeckung.
- [ ] **CR-CRM-008:** Durable Dispatch-/Outbox-Retry ohne neue Schemaänderung; PHPUnit für Commit, Dispatch-Ausfall, Worker-Retry, terminale Auditierung und Erfolg.
- [ ] **CR-PAY-013/CR-CODE-002:** Purchase-time-Organisationssnapshot und Legacy-Fallback; PHPUnit für Reassignment-/Summary-/Fail-closed-Fälle.
- [ ] **CR-DATA-018/CR-BE-018:** Contract-E-Mail-Uniqueness nur mit bestehender Schema-/Lock-Lösung; sonst dokumentierte V039+-Entscheidungsgrenze.
- [ ] **P1-F/P1-A/P1-I/P1-L:** unabhängige UI-, AI-, Supply-Chain- und Plugin-Blöcke mit jeweils Vitest/PHPUnit/Static-/Harness-Nachweis.

**Working-Tree-Verifikation (noch nicht committed):**
- Frontend `pnpm lint:fix`, `pnpm build` und `pnpm test:run`: **119 Testdateien / 935 Tests, 0 Fehler**; `pnpm lint:e2e`, Test-TypeScript und i18n-Check ebenfalls grün.
- Plugin-Harness `bash admin.lrplugin/tests/run.sh`: grün; echte Lightroom-Runtime bleibt nicht verfügbar.
- V037: MariaDB-11.4-Migration im CI-nahen Docker-Netzwerk inklusive Seed und Owner-Trigger-Direktprüfung grün; Retry-Sicherheitspatch und fokussierte 23-Assertion-Regression grün.
...
- Backend-Full-Suite-Lauf 2026-09-24: **umgebungsbedingt blockiert** (Meilisearch `127.0.0.1:7701`, Mailpit `1025/8025` und untracked `backend/.env` fehlten; 813 Aggregatfehler vor Assertions). Kein Produkturteil daraus; offizielle CI-Service-/Container-Baseline bleibt die maßgebliche Referenz.
- CI-Run `36024267904` auf `e1f397d`: **abgeschlossen rot**; Backend (1.972 Tests), Security-Contract und Frontend (111/890) grün, alle sieben E2E-Shards rot. Der nächste Lauf wird nach dem fokussierten CI-Commit gestartet.
...
- **CI-Status 2026-09-24:** Der historische Run `36013526580` auf `59f9ec6` bleibt rot (V037/MariaDB-Fehler 1901; Frontend/Backend ansonsten grün). Der CI-/Migrations-Fix `e1f397d` ist gepusht; Run `36024267904` ist abgeschlossen rot, weil die damals noch fehlenden E2E-/Harness-Fixes nun sichtbar wurden. Image-Build `36024267953` ist grün; ein neuer Run wird erst nach unabhängiger Verifikation der aktuellen Fixes gestartet.
...
- **Frontend-Full-Gate 2026-09-24:** `pnpm test:run` **111 Testdateien / 890 Tests, 0 Fehler**; `pnpm lint:fix`, `pnpm lint:e2e`, `pnpm build` (TypeScript/i18n/Vite) und Plugin-Harness sind grün. Der erste 120-s-Lauf war nur ein Tool-Timeout; der Lauf mit erweitertem Timeout ist vollständig abgeschlossen.
...
- **CR-CODE-002 (P1, purchase-time decision implemented):** `InvoiceService` and the organization summary now prefer the immutable `invoice_snapshots.customer_details.org_id` captured at purchase time; current `users.org_id` membership is used only for legacy snapshots without the key, while present-but-invalid keys fail closed. The existing JSON snapshot column is sufficient; no migration (and no V035 change) is required.
...
- **CR-PAY-013/CR-CODE-002 (P1, implemented; focused backend verification):** Collective invoice selection, the organization summary count, and generated collective snapshots use purchase-time organization attribution; current membership remains only the explicit legacy fallback. `PurchaseTimeOrganizationInvoiceTest` (5 cases) and `CollectiveInvoiceOrganizationAttributionTest` (3 cases) pass with 35 assertions; targeted syntax and Pint checks pass. No migration was added and V035 remains untouched.
...
- [ ] **P1-I3 (MEDIUM; Filter-Fix statisch verifiziert, Branch-Protection/Live-Merge offen):** `automerge.yml` übergibt Dependabot-Metadaten sicher per `env`; `check-regexp` deckt ausschließlich die 10 aktuellen CI-Check-Namen ab, `allowed-conclusions` bleibt `success` und `fail-on-no-checks` ist durch den Filter fail-closed. `actionlint` v1.7.12, Security-Contract, Shell-Syntax und Diff-Check sind grün; ein echter Auto-Merge-Lauf und Branch-Protection werden daraus nicht abgeleitet.
...
- [ ] **CR-PAY-010 (in Umsetzung):** Persistente Request-Deduplizierung/-Idempotenz für nicht-Immediate-Checkout-Nebenpfade ohne PaymentIntent (Rechnung/Lieferschein, settled-free und reaktive Quote-Anfragen) ist als Folgeentscheidung ausgewählt. Der Browser-Key wird serverseitig ausgewertet; Abschluss erst nach Replay-, Konflikt-, Actor-/Brand- und Regressionstests.
