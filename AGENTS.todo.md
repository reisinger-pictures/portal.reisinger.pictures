...
- [x] **TST-5 (aus der Backend-CI-Reparatur 2026-09-26):** `paratest` isoliert die **Datenbank** pro Worker (SQLite `:memory:`), aber **nicht das Dateisystem**. `backend/AGENTS.md` beschreibt die Parallel-Regeln ausschließlich über die DB — diese Lücke ist zweimal real aufgetaucht und hat je einen intermittierenden Fehlschlag verursacht.
  ...
  - **Noch offen:** `PhotoDownloadControllerTest`, `MediaLateAuthorizationTest`, `ManualDocumentTest` und `AIServiceTempFileLocationTest` setzen `filesystems.temp_dir` nicht und schreiben weiterhin in die reale Basis. Sie sind durch die Subordner gegen die anderen Consumer abgesichert, teilen sich aber untereinander weiterhin den Basis-Namenraum. Nach demselben Muster zu isolieren.
