...
- [ ] **TST-5 (neu, aus der Backend-CI-Reparatur 2026-09-26):** `paratest` isoliert die **Datenbank** pro Worker (SQLite `:memory:`), aber **nicht das Dateisystem**. `backend/AGENTS.md` beschreibt die Parallel-Regeln ausschließlich über die DB — diese Lücke ist jetzt zweimal real aufgetaucht und hat je einen intermittent Fehlschlag verursacht. Systematisch offen ist nur noch die **Test-Seite**. Production-seitig ist der Schlüssel inzwischen flächendeckend eingeführt: `PhotoDownloadController.php:140,310,540,938`, `AIService.php:473`, `CleanupTempFiles.php:33` und `ImportLocations.php:74` lesen alle `config('filesystems.temp_dir', storage_path('app/private/temp'))` — **mit Default-Argument**, weil ein reiner `config()`-Aufruf bei veraltetem `config:cache` `null` liefert und in der Dateiauslieferung ein leerer Pfad stiller schlägt als der Produktionspfad. **(b)** Fünf weitere Testklassen teilen sich weiterhin das reale Temp-Verzeichnis: `ImportLocationsTest`, `StorageCommandsTest` (führt `app:cleanup-temp` aus und leert damit den **ganzen** Ordner), `MediaLateAuthorizationTest`, `ManualDocumentTest`, `PhotoDownloadControllerTest`, `AIServiceTempFileLocationTest`. Nur `ImportLocationsNonDestructiveTest` ist bisher isoliert. **Merksatz:** jeder Test, der eine Datei in einem absoluten Shared-Path anlegt, ist unter `--parallel` ein potentielles Opfer eines gleichzeitigen Sweeps. **Zweiter, ähnlicher Fall außerhalb des Dateisystems:** `MailpitAssertions::getMailpitMessageByEmail()` ist behoben (recipient-scoped `/search` statt ungefiltertem `/messages`, das auf die neuesten 50 Nachrichten begrenzt ist) — das war eine **langlebige Ressource**, die nur lokal auftreten konnte, weil CI mit leerem Mailpit startet. Dieselbe Klasse von Fehler ist bei geteilten Diensten (Mailpit, Meilisearch, Redis) generell zu prüfen.
...
**Dependabot-Stand (2026-09-26, Ende der Welle):** 7 Alerts (4 high), 3 PRs — alle 3 sind erledigt, aber **nicht auf dem ursprünglichen Weg.**
