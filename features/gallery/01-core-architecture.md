...
- **Media-/Licensing-Queries:** Der Volume-Preset-Guard (`assertPresetForBrand()`) prüft gegen die **resultierende Galerie-Brand**, nicht mehr gegen die Request-Host-Brand. Damit ist ein `srp`-Preset in einer `srp`-Gruppe auch dann erlaubt, wenn der Super-Admin vom `rp`-Host aus arbeitet — vorher wäre er fälschlich abgelehnt worden. `Gallery::getEffectiveLicensingModeAttribute()` liest bereits `settings.pricing_strategy` über `$this->brand` (mit Host-Fallback nur für Legacy-Null-Brand-Rows).
