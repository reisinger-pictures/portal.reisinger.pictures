...
- **`backend/AGENTS.md`** — Laravel PHP: backend test command, Database Setup + Migration policy (seed after every migration; V038 is the current repository frontier, V037 is the preceding guest-ownership migration, V036 remains the Card-Testing migration, and V035 is the last recorded deployed migration; new changes start at V039), backend parallel testing/paratest rules and worker-DB concurrency.
